...
export default function AccountPage2({account, tokens}: Props) {
  ...
  const handleChangeAmount = (event: any) => setApproveAmount(event.target.value);

  const TokenBalance = (prop: any) => {
    const [balance, setBalance] = useState<any>();

    useEffect(() => {
      getBalance(web3, prop.token.address, account).then(val => setBalance(parseFloat(val).toFixed(5)));
    }, []);
  ...
  return (
    <Box>
      ...
      Amount: <Input placeholder="0" value={approveAmount} onChange={handleChangeAmount} type="number"/>
      {tokens.map((token) =>  (
